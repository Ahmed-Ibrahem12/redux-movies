...
const CastMovie = () => {
  ...
  const Art = MovieCast?.crew?.filter((p) => p.department == "Art");
  const Directing = MovieCast?.crew?.filter((p) => p.department == "Directing");
  const Production = MovieCast?.crew?.filter(
    (p) => p.department == "Production"
  );
  const Crew = MovieCast?.crew?.filter((p) => p.department == "Crew");
  const Camera = MovieCast?.crew?.filter((p) => p.department == "Camera");
  ...
  useEffect(() => {
    setTimeout(() => {
      dispatch(getPersonDetails(MovieId));
